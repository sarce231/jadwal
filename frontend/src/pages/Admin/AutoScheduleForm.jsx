...
const AutoScheduleForm = () => {
  ...
  const createAutoSchedule = (wls, gamesLeaders, places, locations) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const schedule = createAutoSchedule(wls, gamesLeaders, places, locations);
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Jadwal Kegiatan", 14, 16);
    ...
    autoTable(doc, { 
      head: [["No", "Tempat", "Hari, Tanggal", "Lokasi", "WL", "Games Leader", "Waktu"]],
      body: tableData,
      startY: 20,
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-xl p-6 space-y-4 mb-8">
        {[
          { label: "Daftar WL", value: wlListInput, setter: setWlListInput, placeholder: "Contoh: Andi, Budi, Citra" },
          { label: "Daftar Games Leader", value: gamesLeaderListInput, setter: setGamesLeaderListInput, placeholder: "Contoh: Dewi, Eka, Farhan" },
      ...
      {editingItem && (
        <form onSubmit={handleEditSubmit} className="bg-yellow-100 p-6 rounded-xl space-y-4 mb-8 shadow-inner">
          ...
          {["place", "location", "date", "day", "time", "wl", "gamesLeader"].map((field) => (
            <div key={field}>
      ...
      {generatedSchedule.length > 0 && (
        <div className="overflow-x-auto">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">Jadwal Kegiatan</h2>
              ...
              {generatedSchedule.map((item, index) => (
                <tr key={item._id} className="border-t">
                  ...
                  <td className="py-2 px-4 space-x-2">
                    <button onClick={() => handleEdit(item)} className="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">
                      Edit
                    </button>
