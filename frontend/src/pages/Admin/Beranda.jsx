...
const Beranda = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [jadwalRes, mediaRes, penggunaRes, komentarRes] = await Promise.all([
          fetch('http://localhost:5000/api/schedules'),
          fetch('http://localhost:5000/api/media'),
          fetch('http://localhost:5000/api/admin'),
          fetch('http://localhost:5000/api/chats/total-messages') // pakai endpoint baru
        ]);
  
        const jadwalData = await jadwalRes.json();
        const mediaData = await mediaRes.json();
        const penggunaData = await penggunaRes.json();
        const komentarData = await komentarRes.json(); // { totalMessages: X }
  
        setJumlahJadwal(jadwalData.length || 0);
        setJumlahMedia(mediaData.length || 0);
        setJumlahPengguna(penggunaData.length || 0);
        setJumlahKomentar(komentarData.totalMessages || 0); // Ambil totalMessages
      } catch (error) {
  ...
  const chartData = {
    labels: ['Jadwal', 'Media', 'Pengguna', 'Komentar'],
    datasets: [
      {
        label: 'Jumlah',
        data: [jumlahJadwal, jumlahMedia, jumlahPengguna, jumlahKomentar],
  ...
  return (
    <div className="container mx-auto p-4 overflow-y-auto" style={{ height: '100vh' }}>
      <div className="p-6 bg-gray-50 min-h-screen">
        <h2 className="text-3xl font-bold mb-8 text-gray-800">Dashboard Admin</h2>
